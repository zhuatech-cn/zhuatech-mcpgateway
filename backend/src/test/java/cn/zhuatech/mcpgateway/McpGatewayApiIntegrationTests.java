 ...
 */
@SpringBootTest @AutoConfigureMockMvc class McpGatewayApiIntegrationTests {
     ...
     * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
     */
    @Test void operatorCanReadShopfloorDashboard()throws Exception{mvc.perform(get("/api/shopfloor/dashboard").header("Authorization","Bearer "+operatorToken)).andExpect(status().isOk()).andExpect(jsonPath("$.success").value(true)).andExpect(jsonPath("$.data.metrics[0].label").value("注册 MCP 工具"));}
    /**
     * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
     */
     ...
     * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
     */
    @Test void operatorCanSubmitProductionReport()throws Exception{mvc.perform(post("/api/shopfloor/work-orders/1/reports").header("Authorization","Bearer "+operatorToken).contentType(MediaType.APPLICATION_JSON).content("{\"operationName\":\"策略联调\",\"goodQty\":2,\"defectQty\":1,\"remark\":\"审计字段完整\"}")).andExpect(status().isOk()).andExpect(jsonPath("$.message").value("网关处置反馈已提交")).andExpect(jsonPath("$.data.completedQty").value(18));}
    /**
     * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
     */
    @Test void operatorCanEvaluateMcpCall()throws Exception{mvc.perform(post("/api/shopfloor/access-decision").header("Authorization","Bearer "+operatorToken).contentType(MediaType.APPLICATION_JSON).content("{\"serverName\":\"mail-mcp\",\"toolName\":\"send_mail\",\"userRole\":\"USER\",\"readOnly\":false,\"sensitiveData\":true,\"externalWrite\":true,\"approvedServer\":false,\"scopedCredential\":false}")).andExpect(status().isOk()).andExpect(jsonPath("$.data.decision").value("BLOCK"));}
    /**
     * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
     */
